...
% where:
% 其中：
% ecv = H - Hc, denotes the Common-Voltage-Reduction Error;
%   ecv = H - Hc，表示同电压压缩误差；
% eal = Hc - Heq, denotes the Admissible-Law-Approximation Error.
%   eal = Hc - Heq，表示允许函数类近似误差。
...
% Consistent-branch approximation of the Common-Voltage-Reduction Error
% 一致分支同电压压缩误差近似
% ecv_50 = bcv + YpI * inv(M_I^alpha) * rI_alpha
ecv_consistent_approx_vec = zeros(size(Vp_abs_vec));

% Inconsistent-branch approximation of the Common-Voltage-Reduction Error
% 不一致分支同电压压缩误差近似
% ecv_53 = bcv + YpI * inv(M_I^alphaStar) * (rI_alpha + mI)
ecv_branch_approx_vec = zeros(size(Vp_abs_vec));

% Branch-adaptive approximation of the Common-Voltage-Reduction Error:
% 分支自适应同电压压缩误差近似：
...
%% ============================================================
% 9.2 Admissible-Law-Approximation Error approximation variables
%  9.2 允许函数类近似误差近似变量
% ============================================================
% Admissible-Law-Approximation Error:
% 允许函数类近似误差：
%
%   eal = Hc - Heq

% Consistent-branch approximation of the Admissible-Law-Approximation Error
% 一致分支允许函数类近似误差近似
% eal_60 = -Yeq_pI * inv(M_e^beta) * delta_ab
eal_consistent_approx_vec = zeros(size(Vp_abs_vec));

% Inconsistent-branch approximation of the Admissible-Law-Approximation Error
% 不一致分支允许函数类近似误差近似
% eal_63 = -Yeq_pI * inv(M_e^betaStar) * (delta_ab + me)
eal_func_approx_vec = zeros(size(Vp_abs_vec));

% Branch-adaptive approximation of the Admissible-Law-Approximation Error:
% 分支自适应允许函数类近似误差近似：
...
for k = 1:length(Vp_abs_vec)
    ...
    [ecv_consistent_approx, ecv_branch_approx, mI_aug, rI_alpha_aug, condM, ...
    ...
    [eal_consistent_approx, eal_func_approx, delta_ab, me_beta, condMe, ...
...
% English note: Error-decomposition check |(ecv+eal)-etot| 在 Vp=1 时 = %.4e\n
fprintf('误差分解校验 |(ecv+eal)-etot| 在 Vp=1 时 = %.4e\n', ...
    abs((ecv_plot(idx1) + eal_plot(idx1)) - etot_plot(idx1)));
% English note: Number of Common-Voltage-Reduction branch mismatches Vp=1 时 = %d\n
fprintf('同电压压缩分支不一致数量 Vp=1 时 = %d\n', branchMismatchCount_plot(idx1));
% English note: 式(50)approximation error |ecv_50-ecv| Vp=1 时 = %.4e\n
fprintf('式(50)近似误差 |ecv_50-ecv| Vp=1 时 = %.4e\n', ...
    abs(ecv_consistent_approx_plot(idx1) - ecv_plot(idx1)));
% English note: 式(53)approximation error |ecv_53-ecv| Vp=1 时 = %.4e\n
fprintf('式(53)近似误差 |ecv_53-ecv| Vp=1 时 = %.4e\n', ...
...
% English note: Admissible-Law-Approximation residual |delta_alpha_beta| Vp=1 时 = %.4e\n
fprintf('允许函数类近似残差 |delta_alpha_beta| Vp=1 时 = %.4e\n', delta_ab_norm_plot(idx1));
% English note: Equivalent-side branch-switching residual |me| Vp=1 时 = %.4e\n
fprintf('等值分支切换残差 |me| Vp=1 时 = %.4e\n', me_norm_plot(idx1));
% English note: 式(60)approximation error |eal_60-eal| Vp=1 时 = %.4e\n
fprintf('式(60)近似误差 |eal_60-eal| Vp=1 时 = %.4e\n', ...
    abs(eal_consistent_approx_plot(idx1) - eal_plot(idx1)));
% English note: 式(63)approximation error |eal_63-eal| Vp=1 时 = %.4e\n
fprintf('式(63)近似误差 |eal_63-eal| Vp=1 时 = %.4e\n', ...
...
% English note: branch自适应 |ecv_piecewise-ecv| Vp=1 时 = %.4e\n
fprintf('分支自适应 |ecv_piecewise-ecv| Vp=1 时 = %.4e\n', ...
    abs(ecv_piecewise_approx_plot(idx1) - ecv_plot(idx1)));
% English note: branch自适应 |eal_piecewise-eal| Vp=1 时 = %.4e\n
fprintf('分支自适应 |eal_piecewise-eal| Vp=1 时 = %.4e\n', ...
...
rms_ecv  = sqrt(mean(y_ecv.^2));
rms_eal  = sqrt(mean(y_eal.^2));

fprintf('|e_tot| RMS = %.6e p.u.\n', rms_etot);
fprintf('|e_cv | RMS = %.6e p.u.\n', rms_ecv);
fprintf('|e_al | RMS = %.6e p.u.\n', rms_eal);
%% ============================================================
...
%% ========================================================================
% Approximate calculation of the Common-Voltage-Reduction Error formula
%  同电压压缩误差式近似计算
% ========================================================================

function [ecv_consistent_approx, ecv_branch_approx, mI_aug, rI_alpha_aug, condM, ...
    condMIAlpha, condMIAlphaStar, ...
    branchMismatchCount, branchCommon, branchTrue, ...
    KvdAlpha_real, KvdAlphaStar_real] = ...
    compute_inconsistent_branch_approx( ...
...
%% ========================================================================
% Approximate calculation of Admissible-Law-Approximation Error using Eq. (60) and Eq. (63)
%  允许函数类近似误差式（60）和式（63）近似计算
% ========================================================================

function [eal_consistent_approx, eal_func_approx, delta_ab, me_beta, condMe, ...
    condMeBeta, condMeBetaStar, ...
    eqBranchMismatch, branchBeta, branchBetaStar, ...
    KfsBeta_real, KfsBetaStar_real] = ...
    compute_function_structure_approx( ...
...
% 6. 式（63）不一致分支近似
% ------------------------------------------------------------
% eal_63 ≈ -Yeq_pI * inv(M_e^betaStar) * (delta_ab + me_beta)
